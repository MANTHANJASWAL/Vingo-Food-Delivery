...
function OwnerItemCard({ data }) {
  ...
  const handleDelete=async ()=>{
    const previousShopData=myShopData
    try {
      const currentItems=myShopData?.Items || myShopData?.items || []
      dispatch(setMyShopData({
        ...myShopData,
        Items:currentItems.filter(item=>item._id!==data._id),
        items:currentItems.filter(item=>item._id!==data._id)
      }))
      ...
      console.log(error)
      dispatch(setMyShopData(previousShopData))
    }
  }
      ...
      <div className="flex flex-col justify-between p-3 flex-1">
        <div>
          ...
          <p>
            ...
            {foodTypeLabels[data.foodType] || data.foodType}
          </p>
        </div>
